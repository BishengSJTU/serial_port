...
const SerialPort::OpenOptions SerialPort::defaultOptions = {
    true, //        bool autoOpen;
    SerialPort::BR38400, //    BaudRate baudRate;
...
int SerialPort::write(const unsigned char *data, int length) {
    return ::write(_tty_fd, data, length);
}

int SerialPort::write(const vector<unsigned char> vData)
{
    std::shared_ptr<unsigned char> p(new unsigned char[vData.size()]);
    for(int i = 0; i < vData.size(); i++) {
        p.get()[i] = vData[i];
    }
    return ::write(_tty_fd, p.get(), vData.size());
}
